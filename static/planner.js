...
async function loadProjects(){
  try {
    ...
    console.log('[planner] /api/planner/projects ->', res);
    ...
    console.error('[planner] loadProjects error', e);
...
function renderRoadmap(project, tasks){
  ...
  tasks.forEach((t, i)=>{
    ...
    if(t.completed){
      // For completed tasks, extract the main name and completion date
...
function showDetail(project, task, nodeEl){
  ...
  // Показываем информацию о повторе в отдельном месте
  ...
  document.getElementById('detailTextarea').value = displayContent;  // сохраняем чистое содержимое для редактирования
...
console.log('[planner] script init');
loadProjects();

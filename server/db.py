...
def init_db(db_path: str = 'habits.db'):
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS ideas (
            ...
            title TEXT NOT NULL,
            description TEXT,
            realism INTEGER DEFAULT 5,
            related_ids TEXT,  -- JSON-массив id связанных идей
            i REAL DEFAULT 0,
            s REAL DEFAULT 0,
            w REAL DEFAULT 0,
            e REAL DEFAULT 0,
    ...
    # Добавление новых полей в таблицу ideas (если их ещё нет)
    try:
        cursor.execute("ALTER TABLE ideas ADD COLUMN idea_type TEXT DEFAULT 'own'")
    except sqlite3.OperationalError:
        pass  # колонка уже существует
    try:
        cursor.execute("ALTER TABLE ideas ADD COLUMN source TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        cursor.execute("ALTER TABLE ideas ADD COLUMN problems TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        cursor.execute("ALTER TABLE ideas ADD COLUMN what_changes TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        cursor.execute("ALTER TABLE ideas ADD COLUMN difficulty TEXT DEFAULT 'background'")
    except sqlite3.OperationalError:
        pass
